...
int UdpSocketClient::recv()
{
	...
	int len = recvfrom(sockfd, buf, sizeof(buf), 0, (struct sockaddr *)&server_addr, &addrlen);
	if (len == -1) {
		perror("recv() failed");
		return -1;
	}
...
{
	if (sendto(sockfd, buf, size, 0, (struct sockaddr *)&server_addr, sizeof(server_addr)) == -1) {
...
int main(int argc, char **argv)
{
	...
	char buf[] = "Hello this message is from client";
	int i = 1;

	while (1) {
		cout << "[" << i++ << "] ";
		if (udp_client.send(buf, strlen(buf)) == -1)
			break;
